              ...
              {students.map((s) => (
                <tr key={s.id} className="table-row">
                  <td className="px-5 py-3">
                    <button onClick={() => viewStudent(s)} className="text-sm text-white font-medium hover:text-primary-400 transition-colors">
                      {s.name}
                    </button>
                  </td>
                  <td className="px-5 py-3 text-sm text-dark-300">{s.enrollmentNo}</td>
                  <td className="px-5 py-3 text-sm text-dark-300">{s.department}</td>
                  <td className="px-5 py-3 text-sm text-dark-400">{s.email}</td>
                  <td className="px-5 py-3 text-sm text-dark-400">{s.rfidUid || '—'}</td>
                  ...
                  <td className="px-5 py-3 text-center">
                    <div className="flex items-center justify-center gap-2">
                      <button onClick={() => openEdit(s)} className="text-xs text-primary-400 hover:text-primary-300 font-medium">Edit</button>
                      <button onClick={() => handleDelete(s.id)} className="text-xs text-red-400 hover:text-red-300 font-medium">Deactivate</button>
                    </div>
                  </td>
                </tr>
              ))}
              {students.length === 0 && (
                <tr><td colSpan="7" className="px-5 py-8 text-center text-dark-400">No students found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="modal-overlay" onClick={() => setSelectedStudent(null)}>
          <div className="modal-content max-w-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-dark-700">
              ...
              <button onClick={() => setSelectedStudent(null)} className="p-2 hover:bg-dark-700 rounded-lg transition-colors">
                <HiOutlineX className="w-5 h-5 text-dark-400" />
              </button>
            </div>
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div><span className="text-dark-400">Email:</span> <span className="text-white ml-2">{selectedStudent.email}</span></div>
                <div><span className="text-dark-400">Phone:</span> <span className="text-white ml-2">{selectedStudent.phone || '—'}</span></div>
                <div><span className="text-dark-400">Dept:</span> <span className="text-white ml-2">{selectedStudent.department}</span></div>
                <div><span className="text-dark-400">RFID:</span> <span className="text-white ml-2">{selectedStudent.rfidUid || '—'}</span></div>
              </div>
              <h3 className="text-white font-semibold pt-4 border-t border-dark-700">Issue History</h3>
              {studentIssues.length > 0 ? (
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {studentIssues.map((issue) => (
                    <div key={issue.id} className="flex items-center justify-between p-3 bg-dark-900/50 rounded-xl text-sm">
                      <div>
                        <p className="text-white font-medium">{issue.book?.title}</p>
                        <p className="text-xs text-dark-400">{new Date(issue.issueDate).toLocaleDateString()} → {issue.returnDate ? new Date(issue.returnDate).toLocaleDateString() : 'Not returned'}</p>
                      </div>
                      <span className={
                ...
                <p className="text-dark-400 text-sm">No issues found.</p>
              )}
            </div>
          </div>
        </div>
      ...
      {showModal && (
        <div className="modal-overlay" onClick={closeModal}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-dark-700">
              <h2 className="text-lg font-bold text-white">{editStudent ? 'Edit Student' : 'Add Student'}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm text-dark-300 mb-1">Enrollment No. *</label>
                ...
                <div>
                  <label className="block text-sm text-dark-300 mb-1">RFID UID</label>
